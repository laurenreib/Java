...
public class FireSimulationGUI extends Application implements Observer {
    public static FireSimulationGUI gui;
    private Sensor Network;
    private final static int WINDOW_WIDTH = 1000;
    private final static int WINDOW_HEIGHT = 800;
    private final static double NODE_RADIUS = 10;
    private final static double GRID_SPACING = 85.0;

    private static Map<String, Circle> nodes =
            Collections.synchronizedMap(new HashMap<>());
    private static Map<String, Circle> agents =
            Collections.synchronizedMap(new HashMap<>());
    ...
    private final Queue<String> updates;
    public static boolean fireStarted = false;
    private Sensor baseStationSensor;
    public FireSimulationGUI () {
        String filename = "sample.txt";
        Network = Configuration.buildSensor("example_config/" + filename);
        baseStationSensor = findBaseStation(Network, new HashSet<>());
        updates = new ConcurrentLinkedQueue<>();
        observeEachNode(Network, this, Collections.synchronizedSet(new HashSet<>()));
    }
    public static void main(String[] args) {
        gui = new FireSimulationGUI();
        launch(args);
    }

    /**
     * Initializes and launches the GUI, sets up the simulation panel,
     * loads sensor network, UI controls, and observer relationships.
     */
    @Override
...
//            fireNodes.clear();
            ...
            //  Activate the fire start node
            Sensor fireLoc = Network.getFireLocation();
            Sensor baseLoc = Network;
            if (fireLoc != null) {
                fireLoc.activate();
                BaseStation bs = new BaseStation(baseLoc);
                bs.run();
            }
    ...
    /**
     * Recursively displays all sensors and edges in the network on the pane.
     * Sets node colors based on their status and tracks fire-starting nodes.
     */
...
//        }
        ...
        nodes.put(x + "," + y, node);
        Circle agent = createAgent(x, y);
        agents.put(x + "," + y, agent);
        simulationPane.getChildren().add(node);
        simulationPane.getChildren().add(agent);
    }
    /**
     *Updates the visual color of a sensor node based on its new status.
     *Called from various status transitions.
     * String is the key that will be used to find the corresponding circle
     */
    public synchronized void changeColor (double x, double y, String color) {
        ...
        Circle target = nodes.get(x + "," + y);
        if(target != null) target.setFill(colour);
    }

    /** Creates a circular node representation on the GUI for a sensor
     */
    ...
    /**
     * Creates a visual line between two sensor nodes representing a connection.
      */
    ...
    /**
     * Recursively attaches the GUI as an observer to each sensor node.
     * Enables live status tracking and visual updates.
      */
    ...
    /**
     * Recursively searches for and returns the BASE_STATION sensor.
     * Used to reference the base station later on.
     */
    private Sensor findBaseStation(Sensor s, Set<Sensor> visited) {
        if (s == null || visited.contains(s)) return null;
        visited.add(s);
        if (s.getStatus().equals(SensorStatus.BASE_STATION)) return s;

        for (Sensor.Node n : s.node.getPtrs()) {
            Sensor result = findBaseStation(n.getOwner(), visited);
            if (result != null) return result;
        }
        return null;
    }
    public synchronized void observeMobileAgent (MobileAgent agent) {
        agent.addObserver(gui);
    }
    public synchronized Circle createAgent(double x, double y) {
        Circle circle = new Circle(x, y, NODE_RADIUS * 1.25);
    ...
    public synchronized void changeAgent (double x, double y, String status) {
        x = x * GRID_SPACING + xOffset;
        y = y * GRID_SPACING + yOffset;
        Circle target = agents.get(x + "," + y);
        Color color = switch (status) {
            case "Alive" -> Color.GREEN.brighter();
            case "Dead" -> Color.TRANSPARENT;
            default -> (Color)target.getStroke();
        };
        if(target != null) target.setStroke(color);
    }
    /**
     * Observer method implementation, called whenever a sensor changes state.
     * Triggers the GUI updates based on sensor status (ie. BURNING,
     * DEAD, ect).
      */
    @Override
        ...
        synchronized (this) {
            if (arg instanceof String message) {
                ...
                if(args[0].equals("Agent")) {
                    ...
                    Platform.runLater(() -> changeAgent(x, y, args[1]));
                    return;
                }

                if (args[0].equals(StationMessages.DISPATCH) ||
                        args[0].equals(StationMessages.BURN) ||
                        args[0].equals(StationMessages.DEATH)) {
                    return;
                }
                ...
                // Always update color to match current status
                Platform.runLater(() -> changeColor(x, y, status));
            }
        }
    }
}
